#' mygmapsdistance is going to try and add parameters from teh Google Maps API to the existing function
...
#' results
mygmapsdistance = function(origin, destination, mode="driving", key = get.api.key()) {
  ...
  Time = as(xmlChildren(results$row[[1]])$duration_in_traffic[1]$value[1]$text, "numeric")
